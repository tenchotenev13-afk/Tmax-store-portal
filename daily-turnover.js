...
/* Изпълнението на задачата идва от ДАННИТЕ, не от твърдение: щом оборотът е
   ...
   ИЗКЛЮЧВАНЕТО ЗА СЕДМИЦА (recurring_task_skips) НАРОЧНО НЕ СЕ ГЛЕДА ТУК.
   Изключена ли е „Вечерен оборот" за седмицата (за всички или за този
   обект), отметката пак се записва. Причините:
     · оборотът е ФАКТ, а отметката е следствие от него — не твърдение, което
       изключването отменя;
     · изключеният обект е извън знаменателя навсякъде (Бюлетин, „Днес",
       отчетите), тоест отметката не се брои и не вреди;
     · натисне ли някой „Върни", задачата отново е дължима — и вече е
       отметната, защото оборотът е подаден. Пропуснем ли отметката тук,
       обектът би излязъл неизпълнил въпреки подадения оборот.
