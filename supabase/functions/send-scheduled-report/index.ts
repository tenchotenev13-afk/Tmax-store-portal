/* send-scheduled-report — Edge Function за АВТОМАТИЧНОТО (cron) изпращане
   ...
   СРОКА. Деплоят носи ТРИ неща, всичките в двата колектора:
     · ДНЕВНИЯТ тегли и многоседмичните за седмицата на отчетния ден
       (loadSpanningTasks) — денят на срока е в ДРУГ бюлетин и без тази
       заявка отчетът за него изобщо не вижда задачата;
     · СЕДМИЧНИЯТ вади тези със срок СЛЕД отчетната седмица от явяванията и
       ги дава в нов списък „🗓 В срок (краен срок по-късно)“ с напредъка
       X/18 — извън числителя и знаменателя, иначе обектът излиза
       неизпълнил работа, която още не се изисква. Шеста заявка НАКРАЯ на
       Promise.all (r2[5]) за техните отметки: completion_date им е срокът,
       тоест извън прозореца на седмицата. Индексите r2[0..4] НЕ се местят;
     · шест нови копия от shared.js (taskSpansWeeks, taskSpanDue,
       taskCountsInWeek, spanWeekSunday, loadSpanningTasks,
       mergeSpanningTasks) + reportSpanDueWeekLabel/reportSpanPendingHtml от
       report.js. tests/report-edge-sync.test.js ги сверява ред по ред.
   ...
   касиер (created_by). Отделна заявка по storno_date за седмицата (и
   бележки, въведени седмици по-късно, влизат); заявката по created_at за
   старите сторно метрики не е пипана. reportSmallStornoHtml е споделена
   с report.js. Таб „Днес" (подвижен прозорец) не получава секцията.

   v33 (13.09.2026) — нов отчет „Палети за прибиране" (body {"type":"pallets"},
   кронът — петък 21:00): по обект от transport_pallets — попълнил е
   най-новият ред от понеделника до отчетния ден (с дата в името, ако не е
   от самия ден); непопълнилите със сивия си последен ред отпреди
   понеделника, извън „Общо". Получатели: регионалните за
   своите обекти и report_recipients с weekly=true (scope_stores важи);
   без управители и без snapshot. Ранен клон в обработчика — пътят на
   дневния/седмичния не е пипан. collectPalletsReportData,
   reportPalletsHtml, reportPalletsSubject и reportPalletsRecipients са
   споделени с report.js.

   v32 (13.09.2026) — отчетите минават от сутрин на 21:00: дневният описва
   ДНЕШНИЯ ден (reportDailyTargetDate без −1; тенденцията сравнява изрично с
   вчерашния snapshot), седмичният — ТЕКУЩАТА седмица през
   reportTargetWeekMonday (бивша reportPrevWeekMonday) и излиза в неделя.
   Кронът се сменя отделно в базата; send-routed-report не е пипан и остава
   на предходната седмица.

   v31 (13.09.2026) — секция „Каса" в дневния: подробно само върнатите ПРЕЗ
   отчетния ден, по-старите непоправени — по един ред на обект („Непоправени
   от по-рано"), а сумите и прагът са в € вместо „лв".

   v30 (12.09.2026) — деплой с ЕДНО нещо: ОТЛАГАНЕ С ТОЧНА ДАТА
   (нова колона task_completions.postponed_to, миграция 20260912152519).

   Денят на едно явяване вече е postponed_to, ако редът е пренесен —
   completion_date остава първоначалният срок, защото е ключът на записа.
   taskDueDateFor() (копие от shared.js) е единственото място, което го
   решава, и минава през reportItemMatchesComp(). Оттам:
     · решетката дава на ПЪРВОНАЧАЛНИЯ ден състояние 'moved' (глиф „→") —
       извън числителя И знаменателя, независимо от статуса; отметнатото
       пренесено явяване пази status='done' върху стария си ред и иначе би
       зачело ден, в който задачата вече не се е очаквала;
     · на НОВИЯ ден явяването е дължимо (done/missing);
     · двата колектора теглят и пренесените В прозореца (отделна заявка по
       postponed_to) и дотеглят по id задачите, които не са в набора —
       иначе отложеното за друга седмица изчезва и от двете;
     · пренесено в ден, в който задачата и без това е дължима → ЕДНО явяване;
     · списъкът „Отложени" носи „→ дд.мм"; стар ред без дата е както досега.
...
/* Копие от report.js — списъкът „в срок“ (многоседмични задачи преди срока
   си). Разминае ли се, имейлът от крона казва друго от портала. */
...
Deno.serve(async (req: Request) => {
  try {
    ...
    /* v33 — „Палети": отделен, по-кратък път. Без праг за каса, без
       ...
       daily/weekly. Изборът кой получава е в reportPalletsRecipients(). */
    if (type === 'pallets') {
      var pData: any = await new Promise(function(resolve){ collectPalletsReportData(null, resolve); });
      if (!pData) {
      ...
      var pStatus = 0;
      if (pPlan.all.length) {
        var pRes = await fetch(SUPABASE_URL + '/functions/v1/resend-email', {
          method: 'POST',
          headers: { 'Content-Type':'application/json', 'Authorization':'Bearer '+SERVICE_KEY, 'apikey':SERVICE_KEY },
          body: JSON.stringify({ to: pPlan.all, subject: pSubject, html: reportPalletsHtml(pData) })
        });
        pOk = pRes.ok;
        pStatus = pRes.status;
      }
      ...
      var pPersonal: any[] = [];
      for (const u of pPlan.personal) {
        if (!u.stores.length) { pPersonal.push({ email:u.email, sent:false, reason:'no_assigned_stores' }); continue; }
        var uData: any = await new Promise(function(resolve){ collectPalletsReportData(u.stores, resolve); });
        if (!uData) { pPersonal.push({ email:u.email, sent:false, reason:'collect_failed' }); continue; }
        if (!uData.storeCount) { pPersonal.push({ email:u.email, sent:false, reason:'no_reportable_stores' }); continue; }
        var uRes = await fetch(SUPABASE_URL + '/functions/v1/resend-email', {
          method: 'POST',
          headers: { 'Content-Type':'application/json', 'Authorization':'Bearer '+SERVICE_KEY, 'apikey':SERVICE_KEY },
          body: JSON.stringify({ to: [u.email], subject: pSubject, html: reportPalletsHtml(uData) })
        });
        pPersonal.push({ email:u.email, stores:uData.storeCount, sent:uRes.ok, status:uRes.status });
      }

      if (!pPlan.all.length && !pPersonal.length) {
