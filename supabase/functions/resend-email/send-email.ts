...
/* Нарочно с цикъл, а не с регекс за ASCII диапазона.

   ВНИМАНИЕ ЗА ДЕПЛОЯ: файлът съзнателно НЕ съдържа нито един u-escape
   (обратна наклонена черта, буква u и четири шестнайсетични цифри).
   Деплойът на Edge Function минава през JSON, където такъв запис Е escape
   и се превръща в самия символ — тоест низът за нулевия символ става СУРОВ
   NUL байт в изходния файл. Функцията пак работи, но файлът се разминава с
   репото и става нечетим за редактори. Случи се четири пъти подред на
   23.08.2026, защото при деплой кирилицата също се праща с такива записи и
   нулевият символ е неразличим от тях. Затова: ако тук потрябва проверка за
   диапазон от кодове, пиши я с charCodeAt, не с регекс с escape-и.
   tests/email-subject-rfc2047.test.js заковава, че файлът няма сурови
   контролни байтове. */
