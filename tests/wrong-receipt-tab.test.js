...
(async function () {
  ...
  section('3. Етикетът НЕ е „Междускладов" — екран, печат, имейл');
  {
    ...
    if (guard('renderDiffPrint() не хвърля', () => w.renderDiffPrint(w.diffReports.find(r => r.id === 'rep-wr')))) {
      ...
      ok('колоната за количество е „Фактура"', p.indexOf('Фактура') >= 0);
      /* 11mm при 7.5pt не побира „По фактура" — пречупваше се на три реда.
         jsdom не смята лейаут, затова се заковава самият низ. */
      ok('и НЕ е дългото „По фактура" (пречупва се на три реда)',
        p.indexOf('По фактура') < 0);
      /* „НЕВЗЕТА" е дума от потока на връщанията — при сторна никой не идва
         да взима стока. Състоянието е дали обектът е изчистил реда. */
      ok('статусът е „НЕИЗЧИСТЕНА", не „НЕВЗЕТА"',
        p.indexOf('НЕИЗЧИСТЕНА') >= 0 && p.indexOf('НЕВЗЕТА') < 0);
    }
  ...
  section('4. Старите две посоки пазят СВОИТЕ етикети (без регресия)');
  {
    ...
    /* SUP_DONE е type='writein' → думата му е „НЕЗАПРИХОДЕНА". Важното е,
       че новата дума не изтича в старите посоки. */
    ok('доставчиковият печат пази своя речник („НЕЗАПРИХОДЕНА")',
      p.indexOf('НЕЗАПРИХОДЕНА') >= 0 && p.indexOf('НЕИЗЧИСТЕНА') < 0, p.slice(0, 0) || undefined);
    ok('sdStatusWords без посока е непроменена',
      w.sdStatusWords('return').pending === 'Невзета' &&
      w.sdStatusWords('writein').pending === 'Незаприходена');

    /* Надписът под „Снимки" за старите две посоки. Смята се от
       DIFF_CATEGORIES, затова изброява точно тези с meta[3]=true. */
    ok('supplier: изброява увредена стока и грешен баркод',
      w.diffPhotoHintText('supplier').indexOf('увредена стока') >= 0 &&
      w.diffPhotoHintText('supplier').indexOf('грешен баркод') >= 0,
      w.diffPhotoHintText('supplier'));
    ok('interstore: добавя и разликата от опаковка',
      w.diffPhotoHintText('interstore').indexOf('разлика от опаковка') >= 0,
      w.diffPhotoHintText('interstore'));
    /* undelivered е с meta[3]=false — заварената редакция го обещаваше в
       надписа, но submitDiffReport() никога не го е изисквал. */
    ok('и НЕ обещава снимки за „липса", която не ги изисква',
      w.diffPhotoHintText('supplier').indexOf('липса') < 0,
      w.diffPhotoHintText('supplier'));
  ...
  section('8. ЦО подава същата бланка успешно (положителният път)');
  {
    ...
    /* Надписът под "Снимки" следва посоката. При сторна нито една категория
       не изисква снимки, значи не бива да изброява чужди случаи. */
    const hint = doc.getElementById('diff-photo-hint');
    if (ok('надписът под „Снимки" съществува', !!hint)) {
      const t = hint.textContent;
      ok('не изброява категории от старите потоци',
        t.indexOf('увредена стока') < 0 && t.indexOf('грешен баркод') < 0 &&
        t.indexOf('разлика от опаковка') < 0 && t.indexOf('липса') < 0, t);
      ok('казва, че снимките са по избор', t.indexOf('по избор') >= 0, t);
    }
