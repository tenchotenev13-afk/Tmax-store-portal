...
(async function () {
  ...
  {
    const { w } = env();
    /* ЧЕТИРИ обекта тук, а не трите от STORES (променено на 02.09.2026).
       От обхвата за регионалните reportRankingIsMeaningful пада и при
       storeCount < 4 — при трима „ТОП 3" и „ИЗИСКВАТ ВНИМАНИЕ" изброяват
       едни и същи обекти в обратен ред. С тристоричния набор проверката
       „ТОП 3 остава само тук" отдолу щеше да пада по причина, която няма
       нищо общо със седмичния имейл.
       STORES не се пипа: цялата останала част от файла мери решетката и
       знаменателите точно върху трите. Четвъртият живее само в тази секция
       и е без нито едно отмятане, тоест не мести никое от другите числа. */
    const s = w.reportBuildSummary(ITEMS, COMPS, STORES.concat(['Сливен']), 0);
