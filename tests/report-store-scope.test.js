...
(async function () {
  ...
  section('6. Едж функцията: общо писмо + личен цикъл');
  {
    ...
    /* Дотук тук стоеше „регионалните се четат САМО за дневния" (пазачът
       `if (type === 'daily')` около източниците). На 08.09.2026 (v18)
       седмичният получи същия личен път и пазачът падна — проверката е
       сменена с обратния ѝ инвариант, а не махната, за да не изглежда
       като изгубена. Подробностите са в tests/report-weekly-scope.test.js. */
    /* Проверката е по ОТСТЪПА, не по отсъствието на низа `if (type ===
       'daily') {`: от v19 такъв клон пак има, но той чете само прага за
       секцията „Каса". Регионалните се четат на върхово ниво в обработчика
       (4 интервала); вкарат ли се обратно в клон, отстъпът става 6. */
    ok('регионалните се четат на върхово ниво, не в клон за дневния',
    ...
    /* Третият аргумент дойде с v19 (прагът за секцията „Каса", прочетен
       веднъж за всички получатели). Обхватът е ВТОРИЯТ и той е това, което
       се заковава тук — затова регексът не изисква затваряща скоба веднага
       след `mine`, иначе всяко следващо разширение на подписа го чупи. */
    ...
    /* Дотук: „седмичният не праща на регионалните". От v18 праща — точно
       това е промяната, затова проверката е обърната, а не изтрита.
       Обхватът на СЕДМИЧНИЯ се мери в tests/report-weekly-scope.test.js;
       тук се заковава само че двата вида отчет минават през един цикъл. */
    ok('седмичният ползва своя събирач в СЪЩИЯ цикъл',
