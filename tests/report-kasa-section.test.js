/* СЕКЦИЯ „КАСА" В ДНЕВНИЯ ОТЧЕТ — върнатите от счетоводството и
   ...
   1. ВЪРНАТИТЕ подробно са само върнатите ПРЕЗ отчетния ден (returned_at
      на D), независимо от датата на самия отчет. По-старите отиват в
      returnedBacklog — мери се в tests/daily-kasa-backlog.test.js.
...
/* Петте записа от заданието. „ПОС 3" е отчет отпреди 3 дни, върнат на D;
   „ПОС 1" носи разминаване 25 на D; „ПОС 2" — 2 на D (под прага);
   равнението на Враца е върнато, но обектът е ИЗВЪН обхвата; равнението на
   Дупница е 50, но за D-1, тоест не е събитие от отчетния ден. */
const KASA_REPORTS = [
  { store_name: 'Гоце Делчев', date: DAY.back3, pos_number: 3, razlika: 1.5,
    status: 'returned', return_reason: 'Липсва бон за сторно',
    returned_at: DAY.D + 'T09:15:00+03:00' },
...
const KASA_ZOBOROT = [
  { store_name: 'Враца', date: DAY.back3, razlika: 0,
    ...
    returned_at: DAY.D + 'T10:00:00+03:00' },
...
(async function () {
  ...
  section('1. Върнати от счетоводството — върнатите на D, в обхвата');
  {
    const h = env();
    ...
    if (ok('колекторът връща обект', !!k)) {
      ...
      ok('върнат днес (days 0)', r.days === 0, 'реално: ' + r.days);
