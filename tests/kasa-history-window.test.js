/* Каса — прозорец от 7 дни в История (kasa.js).
   ...
   Прозорецът се смята от ЛОКАЛНА дата, самостоятелно в kasa.js — той не
   зависи от bulletin.js по никакъв друг повод и тестът зарежда модулите БЕЗ
   него, за да закове това. (До 13.09.2026 today() беше UTC и рано сутрин
   връщаше вчера; вече е местна, но прозорецът пак не бива да зависи от нея.)
  ...
  section('1. Средата и помощниците');
  {
    ...
    if (typeof w.histWindowStart === 'function') {
      ...
      /* Същината на капана — и единствената ДЕТЕРМИНИРАНА проверка за него.
         Сравнение с dayOffset(-7) не върши работа: UTC срез и локалната
...
/* renderKasa() дели по today(), а today() е местна дата (localDateISO в
   shared.js). Редът „днес" трябва да е СЪЩИЯТ местен ден — затова dayOffset(0),
   а не toISOString(). До 13.09.2026 и двете бяха UTC и секция 8 падаше между
   00:00 и 03:00: w0() и dayOffset(-1) излизаха един и същи ден. */
function w0() { return dayOffset(0); }
