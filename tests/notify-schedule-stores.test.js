/* Камбанката 🔔 по редовете в Бюлетина + новата секция „Насрочени напомняния"
   ...
   Датата по подразбиране е locale-чувствителна: UTC срезът (toISOString()) в
   ранните часове по българско време дава вчера. Проверката е закована с
   ФИКСИРАН момент (27.08.2026, 00:30 българско), не с текущия — иначе минава
   вечер и пада сутрин. (До 13.09.2026 и today() беше UTC; вече е местна.)
...
(async function run() {
  ...
  section('4. Дата по подразбиране: локална, не UTC');
  {
    ...
    /* UTC срезът в този момент е вчерашният ден — това е капанът. today() вече
       е местна (localDateISO) и трябва да дава 27-и, както полето. */
    const utcDate = new h.w.Date().toISOString().slice(0, 10);
    ok('UTC срезът в този момент дава ВЧЕРАШНА дата', utcDate === '2026-08-26', utcDate);
    ok('today() дава местния ден', h.w.today() === '2026-08-27', h.w.today());
    ...
    if (ok('полето за дата съществува', !!fld)) {
      ...
      ok('КОНТРОЛ: не е UTC срезът', fld.value !== utcDate,
        'поле=' + fld.value + '  UTC=' + utcDate);
    }
    restore();
    h.close();
  }
