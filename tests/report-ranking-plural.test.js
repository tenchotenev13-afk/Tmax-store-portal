...
(async function () {
  ...
  section('4. Дневният и седмичният имейл наистина я скриват');
  {
    ...
    /* При разлика седмичният пак я показва.
       ...
       ЧЕТИРИ обекта, не три (променено на 02.09.2026). От обхвата за
       регионалните reportRankingIsMeaningful пада и при storeCount < 4:
       при трима „ТОП 3" и „ИЗИСКВАТ ВНИМАНИЕ" изброяват едни и същи обекти
       в обратен ред. Старият тризначен набор оттук нататък щеше да не
       показва класация по ДВЕ независими причини наведнъж и вече нямаше да
       проверява това, за което е писан — че РАЗЛИКАТА в процентите я връща.
       Числото 4 е точно на границата, тоест проверката е и гранична. */
    const mixed = data([st('Раднево', 100), st('Габрово', 0), st('Троян', 50), st('Сливен', 25)]);
    ok('седмичният я връща при разлика', w.buildWeeklyReportHtml(mixed).indexOf('ТОП 3') >= 0);
    /* И обратното, със същите проценти, но с един обект по-малко — за да
       се вижда, че разликата идва от БРОЯ, не от нещо друго. */
    const mixed3 = data([st('Раднево', 100), st('Габрово', 0), st('Троян', 50)]);
    ok('при 3 обекта я няма, макар процентите да се различават',
      w.buildWeeklyReportHtml(mixed3).indexOf('ТОП 3') < 0);
