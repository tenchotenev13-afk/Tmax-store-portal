...
(async function () {
  ...
  section('8. ЯДРОТО: темите на писмата носят датата');
  {
    const { w } = env();

    ok('дневната тема носи датата',
      w.reportDailySubject('2026-08-23') === '📋 ТеМАХ — Дневен репорт 23.08.2026',
      w.reportDailySubject('2026-08-23'));
    /* Водещи нули — иначе „5.1.2026" се сортира и чете различно от
       останалите теми в пощата. */
    ok('водещи нули в деня и месеца',
      w.reportDailySubject('2026-01-05') === '📋 ТеМАХ — Дневен репорт 05.01.2026',
      w.reportDailySubject('2026-01-05'));

    ok('седмичната тема носи диапазона',
      w.reportWeeklySubject(['2026-08-17', '2026-08-18', '2026-08-19', '2026-08-20',
                             '2026-08-21', '2026-08-22', '2026-08-23'])
        === '📊 ТеМАХ — Седмичен репорт 17.08 – 23.08.2026',
      w.reportWeeklySubject(['2026-08-17', '2026-08-18', '2026-08-19', '2026-08-20',
                             '2026-08-21', '2026-08-22', '2026-08-23']));
    ok('и през Нова година',
      w.reportWeeklySubject(['2025-12-29', '2025-12-30', '2025-12-31', '2026-01-01',
                             '2026-01-02', '2026-01-03', '2026-01-04'])
        === '📊 ТеМАХ — Седмичен репорт 29.12 – 04.01.2026');

    /* Липсва ли датата — старата тема, не счупена нова. */
    ok('без reportDate → старата тема',
      w.reportDailySubject(null) === '📋 ТеМАХ — Дневен репорт',
      w.reportDailySubject(null));
    ok('без weekDates → старата тема',
      w.reportWeeklySubject(null) === '📊 ТеМАХ — Седмичен репорт');
    ok('къс масив дати → старата тема',
      w.reportWeeklySubject(['2026-08-17']) === '📊 ТеМАХ — Седмичен репорт');

    /* Счупена дата не бива да дава „NaN.NaN" в тема до управители. */
    const junk = w.reportDailySubject('не-е-дата');
    ok('счупена дата → старата тема, без NaN',
      junk === '📋 ТеМАХ — Дневен репорт' && junk.indexOf('NaN') < 0, junk);
    const junkWk = w.reportWeeklySubject(['x', 'x', 'x', 'x', 'x', 'x', 'x']);
    ok('счупени дати в седмичната → без NaN',
      junkWk.indexOf('NaN') < 0, junkWk);

    /* Един формат на едно място: диапазонът в темата и в подзаглавието
       трябва да са един и същ низ, иначе едно писмо описва седмицата си по
       два различни начина. */
    const wk = ['2026-08-17', '2026-08-18', '2026-08-19', '2026-08-20',
                '2026-08-21', '2026-08-22', '2026-08-23'];
    const vTema = w.reportWeeklySubject(wk).replace('📊 ТеМАХ — Седмичен репорт ', '');
    const vShapka = w.reportWeekRangeLabel(wk).replace('Обобщение за ', '');
    ok('темата и подзаглавието дават един и същи диапазон',
      vTema === vShapka, vTema + '  ≠  ' + vShapka);
  }
