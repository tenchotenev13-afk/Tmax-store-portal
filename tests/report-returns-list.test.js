/* СПИСЪК „НЕВЗЕТА СТОКА ПО ДОСТАВЧИК" в седмичния отчет.
   ...
   5. РЕД = ОБЕКТ в писмото, макар данните да са групи обект+доставчик.
      От 08.09.2026 (v23) двойките се сливат: числата са на горния текстов
      ред, доставчиците на долния. Дотогава един обект се повтаряше по
      петнайсет пъти и списъкът не се четеше.

   ДВЕ АНТИ-ТАВТОЛОГИИ, всяка проверена на 08.09.2026 с ръчно махане:
   · филтърът `status !== 'pending'` в колектора — махне ли се, влиза
     взетият отпреди 25 дни запис и чупи и броя на групите, и подредбата;
   · филтърът на ДОСТАВЧИЦИТЕ по прага в пълния режим — махне ли се,
     Доставчик2 (2 дни) се появява в реда на A и броят позиции става 4
     вместо 3. Секция 5 пада на две проверки.
...
(async function () {
  ...
  /* ── 4. HTML: срязан отчет — ред на ОБЕКТ ────────────────────────────── */
  section('4. Срязан отчет: ред = обект, всички доставчици');
  {
    const h = env();
    const c = await cross(h, SCOPE);
    ...
    /* ДВА реда, не три: A/Доставчик1 и A/Доставчик2 се сливат в един ред
       за обекта. Точно това е промяната. */
    ok('два реда — по един на ОБЕКТ, не на двойка',
       (html.match(/border-bottom:1px solid/g) || []).length === 2,
    ...
    /* Обект A: 3 позиции от Доставчик1 + 1 от Доставчик2 = 4 при двама. */
    ok('A сумира позициите на двамата доставчици',
       html.indexOf('4 поз. при 2 доставчика') >= 0, html);
    ok('и показва най-старата от тях (12, не 2)',
       html.indexOf('най-старата от 12 дни') >= 0);
    /* Обект B: единствен доставчик — единствено число. */
    ok('B е с ЕДИН доставчик и съответната форма',
       html.indexOf('1 поз. при 1 доставчик<') >= 0 ||
       html.indexOf('1 поз. при 1 доставчик</span>') >= 0, html);
    ok('и най-старата му е 20 дни', html.indexOf('най-старата от 20 дни') >= 0);

    /* Вторият текстов ред: доставчиците, по азбучен ред, с брой в скоби. */
    ok('Доставчик1 излиза с броя си', html.indexOf('Доставчик1 (3)') >= 0, html);
    ok('Доставчик2 е без скоби (една позиция)',
       html.indexOf('Доставчик2') >= 0 && html.indexOf('Доставчик2 (') < 0);
    ok('застоялият доставчик е удебелен',
       html.indexOf('<b>Доставчик1 (3)</b>') >= 0, html);
    ok('пресният доставчик НЕ е удебелен',
       html.indexOf('<b>Доставчик2</b>') < 0);
    ok('азбучен ред в долния ред',
       html.indexOf('Доставчик1 (3)</b>, Доставчик2') >= 0, html);

    ok('застоялият обект е с предупредителен цвят', html.indexOf('#C0392B') >= 0);

    /* Подредбата на обектите: 20 преди 12. */
    ok('обектите са подредени по най-старата позиция',
       html.indexOf('най-старата от 20 дни') < html.indexOf('най-старата от 12 дни'));
    h.close();
  }

  /* ── 5. HTML: пълен отчет — само застоялите доставчици ───────────────── */
  section('5. Пълен отчет: само обектите и доставчиците над прага');
  {
    const h = env();
    const c = await cross(h, null);

    const full = h.w.reportReturnsListHtml(c, false);
    ok('три реда при праг 7 (A, B и чуждият обект)',
       (full.match(/border-bottom:1px solid/g) || []).length === 3,
    ...
    /* АНТИ-ТАВТОЛОГИЯ: A има двама доставчици, но само Доставчик1 е над
       прага. Падне ли филтърът на доставчиците, Доставчик2 се появява в
       долния ред и броят позиции става 4. */
    ok('в пълния режим A показва САМО застоялия доставчик',
       full.indexOf('Доставчик2') < 0, full);
    ok('и броят позиции е на показания, не на всички',
       full.indexOf('3 поз. при 1 доставчик') >= 0, full);
    ok('пресните дни ги няма никъде',
       full.indexOf('най-старата от 2 дни') < 0 &&
       full.indexOf('най-старата от 3 дни') < 0);

    /* Праг 15 — A отпада (най-старата му е 12), остават чуждият обект и B. */
    ...
    ok('40 и 20 остават',
       strict.indexOf('от 40 дни') >= 0 && strict.indexOf('от 20 дни') >= 0);
