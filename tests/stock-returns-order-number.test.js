...
(async function run() {
  ...
  section('б) submitSD с тип „Връщане" → POST носи order_number от полето sd-order');
  {
    ...
    if (guard('модалът за нов запис се отваря', () => w.openSDModal(null))) {
      ...
      /* Другата половина на същата проверка: думата за НОВ запис не бива да се
         смени, докато оправяме тази за редакция. */
      ok('нов запис: потвърждението гласи „Добавено!"',
        calls.toast.some(t => String(t) === '✅ Добавено!'), calls.toast.join(' | '));
      ok('нов запис: НЕ казва „Записано!"',
        !calls.toast.some(t => String(t) === '✅ Записано!'), calls.toast.join(' | '));
    }
  ...
  section('и) Провален PATCH към stock_returns → червен toast, но записът минава');
  {
    ...
    if (guard('модалът за редакция се отваря', () => w.openSDModal('l-i'))) {
      ...
      /* Редакция -> "Записано!". Думата се чете от sdEditId, а closeSDModal()
         го нулира, затова потвърждението трябва да излезе ПРЕДИ затварянето.
         Обратният ред дава "Добавено!" и при редакция. */
      ok('редакцията е потвърдена със „Записано!"',
        calls.toast.some(t => String(t) === '✅ Записано!'), calls.toast.join(' | '));
      ok('НЕ казва „Добавено!" при редакция',
        !calls.toast.some(t => String(t) === '✅ Добавено!'), calls.toast.join(' | '));
