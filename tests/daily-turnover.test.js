...
(async function () {
  ...
  section('8. Разминаване под 10% от оборота — минава с предупреждение');
  {
    /* Законен случай: клиент плаща по банка, касиерът маркира „в брой".
       Разликата е реална — блокирането я правеше неподаваема.
       500 при оборот 6732 е 7,4%, тоест под прага от 26.08.2026. */
    const h = await view();
    await submit(h, '6732.00', '6232.00', '0', '250');
    ok('разлика 500 при оборот 6732 (7,4%) → записът МИНАВА', posts(h).length === 1);
    ok('но не мълчаливо — жълт toast с размера',
      h.calls.toast.some(t => /Сумите не се връзват с 500.00 EUR/.test(t)),
    ...
    ok('и след записа разминаването пак се вижда',
      /разминаване 500.00 EUR/.test(h.calls.toast[h.calls.toast.length - 1]),
      JSON.stringify(h.calls.toast));
  }
  {
    /* Същият оборот, разминаване 2000 (29,7%): при стария праг от 50%
       минаваше, сега блокира. Това е СЪЩИНАТА на промяната. */
    const h = await view();
    await submit(h, '6732.00', '4732.00', '0', '250');
    ok('разлика 2000 при оборот 6732 (29,7%) вече БЛОКИРА', posts(h).length === 0);
  ...
  section('9. Над 10% от оборота — блокира');
  {
    ...
    ok('червен toast, който казва И размера, И допустимото',
      h.calls.toast.some(t => /Разминаването е 760443\.75 EUR, допустимо е до 76812\.50 EUR \(10% от оборота\)/.test(t)),
      JSON.stringify(h.calls.toast));
    ok('toast-ът е ЧЕРВЕН', RED.test(toastColor(h)), toastColor(h));
  }
  {
    /* ГРАНИЦАТА, явно: greatest(1, total*0.1). При оборот 1000 прагът е
       точно 100.00 EUR. */
    const h = await view();
    await submit(h, '1000.00', '900.00', '0', '10');
    ok('разминаване ТОЧНО на прага (100.00 при 1000) МИНАВА', posts(h).length === 1);
  }
  {
    /* Един лев над прага — блокира. Двата случая са съседни нарочно:
       заедно заковават, че сравнението е > , а не >=. */
    const h = await view();
    await submit(h, '1000.00', '899.00', '0', '10');
    ok('един лев НАД прага (101.00 при 1000) БЛОКИРА', posts(h).length === 0);
  }
  {
    /* Старият праг би пуснал всичко до 500 при оборот 1000. */
    const h = await view();
    await submit(h, '1000.00', '500.00', '0', '10');
    ok('50% разминаване (минаваше при стария праг) вече БЛОКИРА',
      posts(h).length === 0);
  }
  {
  ...
  }
  {
    ...
    ok('оборот 2 EUR, разминаване 2 EUR → блокира (над 1 и над 10%)',
      posts(h).length === 0);
  }
  {
    const h = await view();
