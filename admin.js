...
function loadStoresAdmin(){
  sbGet('stores','order=name').then(function(data){
    ...
    if(!list.length){
      body.innerHTML='<tr><td colspan="3" style="text-align:center;padding:20px;color:#94a3b8;">Няма магазини.</td></tr>';
      return;
    }
...
function loadUsersAdmin(){
  sbGet('users','order=role,email&select=email,display_name,store_name,role,active').then(function(data){
    ...
    var list=Array.isArray(data)?data:[];
    if(!list.length){
      body.innerHTML='<tr><td colspan="4" style="text-align:center;padding:20px;color:#94a3b8;">Няма потребители.</td></tr>';
      return;
    }
