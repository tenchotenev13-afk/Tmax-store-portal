...
function submitClientOrder(){
  ...
  /* Задна дата НЕ се забранява — отложеното въвеждане е законно. Но откакто
     ...
     Сравнението е с today(), НЕ с TODAY, и е по НИЗ. Двете не са един и същи
     часовник: today() е toISOString() (UTC), а TODAY е локална полунощ, тоест
     между 00:00 и 03:00 (UTC+3) се разминават с един ден. openClientModal()
     попълва c-date именно с today(), затова срещу TODAY всяка нова заявка,
     въведена нощем, получаваше предупреждението, без никой да е въвеждал
     задна дата. Сверява се с часовника, от който идва стойността.
