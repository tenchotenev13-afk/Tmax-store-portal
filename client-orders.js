...
function submitClientOrder(){
  ...
  /* Задна дата НЕ се забранява — отложеното въвеждане е законно. Но откакто
     ...
     Сравнението е с today() и е по НИЗ: openClientModal() попълва c-date именно
     с today(), значи се сверява с часовника, от който идва стойността. До
     13.09.2026 today() беше toISOString() (UTC) и между 00:00 и 03:00 (UTC+3)
     се разминаваше с TODAY (локална полунощ) с един ден — всяка заявка,
     въведена нощем, получаваше предупреждението, без никой да е въвеждал задна
