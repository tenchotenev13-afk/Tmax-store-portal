...
/* ЗАДАЧА „САМО ЗА ИНФОРМАЦИЯ" (task_type='notice')
   ─────────────────────────────────────────────────
   Показва се като ТЕКСТ в Седмичния календар на Бюлетина и НИКЪДЕ другаде:
   няма чекбокс, няма task_completions, не влиза в брояча X/18, в таб „Днес",
   в дневния/седмичния отчет, в личните писма по задачи, в „Незавършени
   задачи", в Анализ, в седмичния дайджест и в push известията.

   Затова един-единствен предикат, а не по едно 'notice' в петнайсет файла:
   пропусне ли се едно място, задачата тихо влиза в нечий знаменател и
   обектът излиза неизпълнил нещо, което няма как да изпълни.

   Ползва се като филтър НА ВХОДА на всеки колектор и рендер:
     tasks.filter(function(t){ return !taskIsNotice(t); })

   Изключение по конструкция е самият календар — той е мястото, където
   notice ТРЯБВА да се вижда, затова там филтърът е обратен (отделен ред без
   чекбокс), а не изхвърляне. Второ изключение (11.09.2026): блокът
   „Постоянни задачи" в bulletin.js показва постоянната notice като сив ред
   без чекбокс — там са ✏️ / „⏸ Спри" / ✕, без които тя не може да се
   редактира от екрана. Броене там няма; loadTasksStats филтрира сам.

   Трите едж функции (send-scheduled-report, send-routed-report,
   bulletin-notify) държат СВОЕ копие — Deno не може да import-не браузърен
   файл. tests/report-edge-sync.test.js заковава, че копията съвпадат с това
   тук за първите две. */
