...
3. **Реални функционални тестове (jsdom), не само преглед на кода** —
   ...
   Ползвам skill-а `.claude/skills/tmax-jsdom-test/` — **не пиша boot
   boilerplate наново**. Копирам `example.test.js` като `test-<фийчър>.js` в
   корена, сменям require-а на
   `require('./.claude/skills/tmax-jsdom-test/harness')` и пренаписвам само
   секциите. Пълното API е в `SKILL.md` на skill-а.
...
**Всичко минава през skill-а `.claude/skills/tmax-jsdom-test/`.** Там са
`harness.js` (boot, истински кликове, селектори), `smoke.js` (бърза проверка на
зареждането) и `example.test.js` (шаблон). Прочитам неговия `SKILL.md`, преди
да пиша тест — не измислям harness наново.
...
Технически бележки, които се плащат скъпо при забравяне:

- `runScripts: 'outside-only'` + ръчен `w.eval()` на всеки файл. **НЕ**
  `'dangerously'` — при него jsdom тръгва да дърпа истинския OneSignal CDN
  скрипт от `index.html`.
- `w.eval()`, не Node `eval` — inline `onclick` вижда само неща на `window`.
- Файловете се зареждат в реда от `index.html` и се слагат И модулите, които
  редактираният файл ВИКА (напр. `client-orders.js` вика `updateBadges()` от
  `notifications.js`). Иначе в изхода тихо стои `ReferenceError`.
- Търсене на бутон САМО в `button`/`span`, никога в контейнер — обвиващият
  `<div>` съдържа текста на всеки бутон в реда и тестът лъже.
