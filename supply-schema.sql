...
-- Миграция: supply_templates (20260916194826)
--
-- ═══ ЗАЩО ═══════════════════════════════════════════════════════════════
-- Обектите подават всяка седмица колко от даден асортимент да им се зареди
-- (първата бланка: Колоранти, 58 артикула от 4 доставчика). Досега - в
-- Excel по имейл. Фронтът е supply.js (Транспорт > 🎨 Зареждане).
--
--   supply_templates      - бланката: етикети на колоните, за кои обекти е
--                           (target_stores null = всички), инструкция;
--   supply_template_items - редовете на бланката (САП, име, доставчик);
--   supply_entries        - попълненото: един ред на артикул × обект × седмица.
--
-- week_start = понеделникът на ISO седмицата (supplyWeekStart() в supply.js,
-- местна дата). qty1/qty2 null = непопълнено, НЕ 0: „0" е отговор („нямаме"),
-- празно поле не е.
--
-- ═══ ЗАВАРЕНИТЕ ДАННИ (правило 9, проверено 16.09.2026) ═════════════════
-- Таблиците са нови - няма засегнати редове. Проверено преди seed-а: и 13-те
-- имена от заданието съвпадат ТОЧНО (не само case-insensitive) с
-- users.store_name и stores.name. Гардът в seed-а все пак пада при липса,
-- вместо да запише непълен списък. „Търговище" е обектът, НЕ „Логистичен
-- склад Търговище" (сравнението е по равенство, не по like).
--
-- ═══ RLS — ИЗРИЧЕН КОМПРОМИС ═══════════════════════════════════════════
-- Включено и на трите таблици, за разлика от anon_all_* навсякъде другаде:
--   supply_templates, supply_template_items - anon: само SELECT;
--   supply_entries                          - anon: SELECT, INSERT, UPDATE.
--   DELETE няма политика никъде -> отказва се за anon.
-- Компромисът: anon ключът е публичен и общ за всички обекти, затова
-- политиките са using(true)/with check(true) - всеки с ключа може да пише
-- ред за ЧУЖД обект и да чете всички обекти. Базата не знае кой е логнатият;
-- обектът се слага от клиента (currentUser.store_name). Ограничението е
-- само, че бланките не могат да се изтрият или пренапишат от браузъра.
-- Редакцията на шаблоните (Етап 3) ще изисква или UPDATE/INSERT политика тук,
-- или едж функция - решава се тогава, не мълчаливо.
--
-- Три нови таблици → огледалото (Живко): mirror-schema.sql и $TableColumns
-- в sync-mirror.ps1.
-- Миграция: supabase/migrations/20260916194826_supply_templates.sql
...
-- Внимание: rollback трие всички попълнени бланки безвъзвратно.

-- ═══════════════════════════════════════════════════════════════════════
-- СНИМКИ + ДВЕ НОВИ БЛАНКИ — приложено на 19.09.2026
-- Миграция: supply_photos_floor_cables (20260919132946)
-- ═══════════════════════════════════════════════════════════════════════
-- photos: снимки-подсказки под указанията във формата на обекта. Файловете
-- са в репото (img/supply/cable-reel-1..3.jpg, от xl/media на примерния
-- Excel в bulletin-files) и се сервират от GitHub Pages - не от Storage.
-- Преди push на img/ URL-ите връщат 404.
--
-- floor-lm е без sap_code на всички артикули - supply.js скрива колоната
-- САП във формата и в матрицата (в Excel остава, празна).
-- Колорантите: active=false ръчно на 19.09.2026, не са част от миграцията.
--
-- Нова колона → огледалото (Живко): mirror-schema.sql и $TableColumns.
-- Миграция: supabase/migrations/20260919132946_supply_photos_floor_cables.sql
