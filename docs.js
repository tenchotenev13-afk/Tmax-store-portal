/* docs.js — Инструкции и документи
   Редактирай САМО тук когато правиш промени по документите. */
...
function renderDocs(){
  ...
  if(!list.length){grid.innerHTML='<div style="grid-column:1/-1;text-align:center;padding:30px;color:#94a3b8;"><div style="font-size:32px;">📁</div><div>Няма документи.</div></div>';return;}
  var icons={SAP:'🖥',Процедури:'📋',Обучения:'🎓',Формуляри:'📝',Друго:'📄'};
  grid.innerHTML=list.map(function(d){
    return '<div style="border:1px solid #e2e8f0;border-radius:10px;padding:14px;cursor:pointer;display:flex;flex-direction:column;gap:6px;" onclick="window.open(''+d.url+'','_blank')">'+
...
function filterDocs(f,btn){
  ...
  if(btn)btn.classList.add('active');renderDocs();
}
function submitDoc(){
