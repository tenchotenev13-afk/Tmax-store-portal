...
function renderBlockView(bl){
  ...
  if(bl.type==='file'&&bl.url){
    return '<a href="'+esc(bl.url)+'" target="_blank" style="display:flex;align-items:center;gap:8px;padding:8px 12px;border:1px solid #e2e8f0;border-radius:7px;text-decoration:none;color:#0f172a;background:#f8fafc;margin-bottom:8px;">'+
...
function renderBlockEdit(bl,deptKey,i){
  ...
  if(bl.type==='text'){
    ...
    if(bl.url){
      html+='<img src="'+esc(bl.url)+'" style="width:100%;border-radius:5px;margin-bottom:4px;">';
    ...
    if(bl.url){
      html+='<div style="display:flex;align-items:center;gap:8px;padding:8px;background:#f8fafc;border-radius:6px;">'+
        ...
        '<a href="'+esc(bl.url)+'" target="_blank" style="font-size:11px;color:#2563eb;">Изтегли</a></div></div>';
    } else {
      html+='<label style="display:flex;flex-direction:column;align-items:center;padding:16px;border:1.5px dashed #cbd5e1;border-radius:7px;cursor:pointer;color:#64748b;font-size:12px;">'+
