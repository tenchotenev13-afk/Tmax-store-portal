...
var RAW_ROOT = 'https://raw.githubusercontent.com/tenchotenev13-afk/Tmax-store-portal/';
var RAW = RAW_ROOT + 'main/';
...
out.push('2. **Изключение — CDN кешът.** `raw.githubusercontent.com` кешира по ПЪТ');
out.push('   за 5 минути. Покаже ли сверката разминаване, а печатът горе е отпреди');
out.push('   по-малко от 10 минути — това е кешът, не остарял пакет. Тогава изтегли');
out.push('   по SHA от таблицата и сравни с него:');
out.push('');
out.push('       ' + RAW_ROOT + (shaOf['CLAUDE.md'] || head) + '/CLAUDE.md');
out.push('');
out.push('   `?cb=` и `Cache-Control: no-cache` не помагат — кешът е по път, не по');
out.push('   URL с query. Пътят по `main` остава ОСНОВНИЯТ; по SHA е резервният.');
out.push('   Две уговорки: сверката по SHA доказва, че пакетът отговаря на своя');
out.push('   комит, не че комитът е още върхът на `main` — при съмнение повтори по');
out.push('   `main` след изтичане на кеша. И носи ли печатът „некомитнати промени",');
out.push('   разлика спрямо блоба е очаквана, не тревога.');
out.push('3. Сверявай по колоната **„последен комит по него"** в таблицата горе, а');
out.push('   не по `sha` на HEAD: HEAD се мести при всяка промяна по портала, дори');
out.push('   когато тези три файла не са пипани.');
out.push('4. Забравеният `npm run brief` е тих провал — пакетът изглежда наред и');
out.push('   отговорите излизат от стар контекст. Точка 1 го хваща сама.');
out.push('5. При въпрос по портала посочвай версията на копието:');
out.push('   „(копие `' + head + '` · ' + stamp().slice(0, 10) + ')".');
out.push('6. Живият код в репото има превес. Противоречи ли нещо тук на това, което');
out.push('   Claude Code чете от диска, вярно е прочетеното от диска.');
out.push('7. Пакетът се генерира с `npm run brief` и не се комитва — прегенерирай го');
